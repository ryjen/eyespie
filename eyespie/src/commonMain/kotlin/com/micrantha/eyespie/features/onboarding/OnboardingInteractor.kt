...
class OnboardingInteractor(
    ...
    override fun afterReduce(intent: OnboardingIntent, previousState: OnboardingState, stateAfterReduce: OnboardingState) {
        when (intent) {
            OnboardingIntent.Done -> if (!previousState.completing) persistCompletionThen(OnboardingOutput.Completed)
            OnboardingIntent.Skip -> if (!previousState.completing) persistCompletionThen(OnboardingOutput.Dismissed)
