...
internal class GenAISemanticInferenceProvider(
        ...
        generationMutex.withLock {
            ...
            try {
                lifecycleMutex.withLock {
                    ...
                    result = genAI.generate(request.toGenAIRequest()).also {
                        it.failureOrCancellation()
                    }
                }
    ...
    override suspend fun markNotConfigured() = transition {
        SemanticInferenceAvailability.NotConfigured
    }

    override suspend fun markInitializing() = transition {
        SemanticInferenceAvailability.Initializing
    }

    override suspend fun markAvailable(capabilities: SemanticInferenceCapabilities) = transition {
        SemanticInferenceAvailability.Available(capabilities)
    }

    override suspend fun markUnavailable(reasonCode: String) = transition {
        SemanticInferenceAvailability.Unavailable(reasonCode)
    }

    override suspend fun markFailed(diagnosticCode: String) = transition {
        SemanticInferenceAvailability.Failed(diagnosticCode)
    }
    ...
    private fun unavailableFailure(): SemanticInferenceUnavailableException? {
        ...
        return if (!closed && current is SemanticInferenceAvailability.Available) null
